...
fn main() {
  ...
  resources.compile_shader(&display,
                           "cube_shadows",
                           "src/shaders/cube_shadows.vertex.glsl",
                           "src/shaders/cube_shadows.fragment.glsl");
  resources.compile_shader(&display,
                           "axis",
  ...
  resources.compile_shader(&display,
                           "shadow_debug",
                           "src/shaders/shadow_debug.vertex.glsl",
                           "src/shaders/shadow_debug.fragment.glsl");
  ...
  let terrain = world.entities.new_entity();
  {
    ...
    world.entities.set_scale(terrain, Scale(na::one::<na::Vector3<f32>>()/3.0));
  ...
  {
    let position = Position(Vector3::new(0.0, 0.3, 0.0));
    world.entities.set_position(cube, position);
    world.entities.add_geometry(cube, Geometry {
      geometry: "hollow_cube",
      program:  "basic",
    });
    world.entities.set_scale(cube, Scale(na::one::<na::Vector3<f32>>()*1.0));
